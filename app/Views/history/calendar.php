...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            eventClick: function (info) {
                // Check if it's a calendar event (only these can be deleted)
                // We're only allowing deletion of events with eventType 'calendar'
                if (info.event.extendedProps.eventType === 'calendar') {
                    const eventId = info.event.extendedProps.idEvent;
                    
                    // Check if idEvent exists and has the correct format
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            fetch(`/calendar/delete/${actualId}`, {
                                method: 'DELETE',
                                headers: {
                    ...
                    // For non-calendar events, just show info
    ...
    if (titleEl) {
        ...
        if (info.event.title.includes("Izin")) {
            ...
            titleHtml = '<i class="fas fa-user-slash  me-1 text-warning"></i> ';
        } else if (info.event.title.includes("Ganti Hari")) {
            titleHtml = '<i class="fas fa-user-slash  me-1 text-warning"></i> ';
        ...
        // Shorten title to a maximum of 154 characters
...
eventMouseEnter: function(info) {
    ...
    // Add the event time to the tooltip
    if (info.event.extendedProps.start_time) {
    ...
    // Create the tooltip with the event details
