...
<script>
// Konversi data remarkSuggestions dari PHP ke JavaScript.
// Array unik diambil dari kolom 'remark' dari data remarkSuggestions.
var remarkSuggestions = <?php echo json_encode(array_values(array_unique(array_column($remarkSuggestions, 'remark')))); ?>;

$(document).ready(function(){
    // Event input di textarea remark
    ...
    // Saat item suggestion diklik, isi textarea remark dan sembunyikan dropdown
    $('#remark-suggestions').on('click', '.suggestion-item', function(){
        $('#remark').val($(this).text());
        $('#remark-suggestions').hide();
    });
    
    // Sembunyikan dropdown saat klik di luar area remark
