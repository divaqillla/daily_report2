...
<div class="pd-ltr-20 xs-pd-20-10">
  <div class="min-height-200px">
          ...
          <div class="card p-3 mb-3">
            ...
            <div class="row">
             ...
             <!-- Ganti input Part No dengan Select -->
              ...
              <div class="col-md-6 mb-3">
                ...
                <input type="text" id="part_name" name="part_name" class="form-control" value="<?= esc($pps['part_name']) ?>">
              </div>
            ...
            <div class="row">
              ...
              <div class="col-md-4 mb-3">
                ...
                <input type="text" id="cust" name="cust" class="form-control" value="<?= esc($pps['cust']) ?>" required>
              </div>
                            <div class="col-md-4 mb-3">
                <label>Model</label>
                <input type="text" id="model" name="model" class="form-control" value="<?= esc($pps['model']) ?>" required>
              </div>
              <div class="col-md-4 mb-3">
                <label>Receive</label>
              ...
              <table class="table table-bordered" id="tableBasic">
                <thead>
                          <tr>
                              ...
                              <th width="8%">Qty</th>
                              <th width="8%">Estimasi Ukuran Part (p x l)</th>
                ...
                <tbody>
                  <?php if (!empty($dies)): ?>
                    <?php foreach ($dies as $index => $die): ?>
                      ...
                      <td>
                        ...
                        <td>
                          <input type="number" name="dies[<?= $index ?>][panjang]" class="form-control mb-1 mr-1" value="<?= esc($die['panjang']) ?>" oninput="calculateLuas(this)">
                          <input type="number" name="dies[<?= $index ?>][lebar]" class="form-control mb-1" value="<?= esc($die['lebar']) ?>" oninput="calculateLuas(this)">
                        </td>
                  ...
                  <?php else: ?>
                    <tr data-index="0">
                      ...
                      <td>
                        <input type="number" name="dies[0][panjang]" class="form-control mb-1 mr-1" oninput="calculateLuas(this)">
                        <input type="number" name="dies[0][lebar]" class="form-control mb-1" oninput="calculateLuas(this)">
                ...
                <table class="table table-bordered" id="tableDie">
                  <thead>
                    <tr>
                      ...
                      <th style="width: 30%">Die Size ( Dapat mengganti jika tidak sesuai kebutuhan)</th>
                    ...
                    <?php if (!empty($dies)): ?>
                      <?php foreach ($dies as $index => $die): ?>
                        <tr data-index="<?= $index ?>">
                          ...
                          <td>
                            <input type="text" name="dies[<?= $index ?>][die_length]" class="form-control mb-1 mr-1" value="<?= esc($die['die_length']) ?>" readonly>
                            <input type="text" name="dies[<?= $index ?>][die_width]" class="form-control mb-1 mr-1" value="<?= esc($die['die_width']) ?>" readonly>
                            <input type="text" name="dies[<?= $index ?>][die_height]" class="form-control mb-1" value="<?= esc($die['die_height']) ?>" readonly>
                          </td>
                      ...
                      <?php else: ?>
                        ...
                        <tr data-index="<?= $index ?>">
                          ...
                          <td>
                            <input type="text" name="dies[<?= $index ?>][die_length]" class="form-control mb-1 mr-1" readonly>
                            <input type="text" name="dies[<?= $index ?>][die_width]" class="form-control mb-1 mr-1" readonly>
                            <input type="text" name="dies[<?= $index ?>][die_height]" class="form-control mb-1" readonly>
              ...
              <table class="table table-bordered" id="tableDieConstruction">
                ...
                <tbody>
                  <?php
                    ...
                    $padOptions = ['FCD55', 'S45C', 'SKD11', 'HMD5', 'SXACE'];
    ...
    <!-- <button type="submit" name="action" value="update" class="btn btn-success">Update Data</button> -->
              ...
              <!-- Tombol Save As New -->
              <button type="submit" name="action" value="save_as_new" class="btn btn-primary">Copy New Data</button>
          ...
          $(document).ready(function(){
            ...
            '</td>' +
                '<td>'+
                  ...
                  '</div>'+
              '</td>'+
              '<td><input type="text" name="dies['+ index +'][qty_dies]" class="form-control" ></td>'+
                '<td><input type="number" name="dies['+ index +'][panjang]" class="form-control mb-1 mr-1" oninput="calculateLuas(this)"><input type="number" name="dies['+ index +'][lebar]" class="form-control mb-1" oninput="calculateLuas(this)"></td>'+
              ...
              var die = '<tr data-index="'+ index +'">'+
                         '<td><input type="text" name="dies['+ index +'][process]" class="form-control die-process" value="'+ processValue +'" readonly></td>'+
                          '<td>'+
                            ...
                            '</select>'+
                          '</td>'+
                          '<td><input type="text" name="dies['+ index +'][die_length]" class="form-control mb-1" readonly><input type="text" name="dies['+ index +'][die_width]" class="form-control mb-1 mr-1" readonly><input type="text" name="dies['+ index +'][die_height]" class="form-control mb-1" readonly></td>'+
...
function updateDCProcessDropdown(rowIndex) {
  ...
  var processJoin = $row.find('select[name="dies['+ rowIndex +'][proses_gang_input]"]').val() || "";
   console.log("dcprosesdropdown" + machine + "proses"+ proses + );
...
function loadPpsImageList(index) {
  fetch("<?= base_url('Pps/listDieConsImg') ?>")
    .then(res => res.json())
    .then(images => {
...
function scrollSlider2(direction, index) {
  ...
  document.addEventListener('DOMContentLoaded', () => {
    <?php if (!empty($dies)): ?>
      <?php foreach ($dies as $index => $die): ?>
        loadPpsImageList(<?= $index ?>, 'clayout');
        loadPpsImageList(<?= $index ?>, 'die_construction');
      <?php endforeach; ?>
    <?php else: ?>
      loadPpsImageList(0, 'clayout');
      loadPpsImageList(0, 'die_construction');
    <?php endif; ?>
  });
