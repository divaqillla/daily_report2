          ...
          <div class="card p-3 mb-3">
            ...
            <div class="row">
              ...
              <div class="col-md-4 mb-3">
                ...
                <input type="text" name="cust" class="form-control" value="<?= esc($pps['cust']) ?>" required readonly>
              </div>
              <div class="col-md-4 mb-3">
                <label>Model</label>
                <input type="text" name="model" class="form-control" value="<?= esc($pps['model']) ?>" required readonly>
              </div>
              <div class="col-md-4 mb-3">
                <label>Receive</label>
            ...
            <div class="row">
              ...
              <div class="col-md-6 mb-3">
                <label>Part No</label>
                <input type="text" name="part_no" class="form-control" value="<?= esc($pps['part_no']) ?>" readonly>
              </div>
              <div class="col-md-6 mb-3">
                <label>Part Name</label>
                <input type="text" name="part_name" class="form-control" value="<?= esc($pps['part_name']) ?>" readonly>
              </div>
            </div>
          </div>
              ...
              <table class="table table-bordered" id="tableBasic">
                <thead>
                          <tr>
                              ...
                              <th width="6%">Qty</th>
                              <th width="15%">Estimasi Ukuran Part (p x l)</th>
                            ...
                            <?php for ($i = 1; $i <= 3; $i++): ?>
                              <div class="proses-wrapper d-flex align-items-center mb-2" style="gap: 8px; width: 100%;">
                                ...
                                <select
                                  ...
                                  onchange="updateProsesHidden(this.closest('.input-proses')), updateDieConstructionRow(<?= $index ?>), updateDCProcessDropdown(<?= $index ?>), togglePieInputsEditable(this.closest('tr')), updateAllRows()"
                                >
                            ...
                            <?php for ($j = 1; $j <= 3; $j++): ?>
                              ...
                              >
                                <input
                                  ...
                                  style="transform: scale(1.3); margin-right: 2px;"
                                  onchange="updateProsesGangHidden(this.closest('.input-proses-gang')), updateAllRows()"
                                ...
                                <select
                                  ...
                                  class="form-control"
                                  style="width: 100%; height: 38px; font-size: 14px;"
                                  onchange="updateProsesGangHidden(this.closest('.input-proses-gang')), updateDCProcessDropdown(<?= $index ?>), updateAllRows()"
                                >
                                  <option value="">Pilih</option>
                                  <?php foreach ($prosesOptions as $p): ?>
                                    <option
...
<script src="https://code.jquery.com/jquery-3.5.1.min.js"></script>
  ...
  '<td>' +
    '<div style="width:100%;">' +
      // Panjang & Lebar
      '<div style="display:flex; margin-bottom:5px; width:100%;">' +
        '<input type="number" name="dies['+ index +'][panjang]" placeholder="p" class="form-control" ' +
              'style="width:48%; margin-right:4%;" oninput="calculateLuas(this)">' +
        '<input type="number" name="dies['+ index +'][lebar]" placeholder="l" class="form-control" ' +
              'style="width:48%;" oninput="calculateLuas(this)">' +
      '</div>' +

      // Panjang Proses
      '<div style="margin-bottom:5px; width:100%;">' +
        '<input type="number" name="dies['+ index +'][panjangProses]" ' +
              'placeholder="Panjang Proses" class="form-control" style="width:100%;">' +
      '</div>' +

      // Diameter & Jumlah Pie
      '<div style="display:flex; width:100%;">' +
        '<input type="number" name="dies['+ index +'][diameter]" placeholder="d pie" ' +
              'class="form-control" style="width:48%; margin-right:4%;" readonly>' +
        '<input type="number" name="dies['+ index +'][jumlahPie]" placeholder="jml pie" ' +
              'class="form-control" style="width:48%;" readonly>' +
      '</div>' +
    '</div>' +
  '</td>' +
...
function togglePieInputsEditable(row) {
    ...
    if ((proses.includes("PIE") || prosesGang.includes("PIE"))) {
        if (inputDiameter) inputDiameter.readOnly = false;
        if (inputJumlahPie) inputJumlahPie.readOnly = false;

        if (inputPanjangProses) {
        ...
        if (inputPanjangProses) inputPanjangProses.readOnly = false;
    }
}
