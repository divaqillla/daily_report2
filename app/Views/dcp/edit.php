                ...
                <tbody>
                  <?php if(isset($finishing3) && is_array($finishing3) && count($finishing3) > 0): ?>
                    ...
                    <tr>
                      ...
                      <td>
                        <input type="number" name="finishing_qty[]" class="form-control" value="<?= esc($row['finishing_qty'] ?? '') ?>" >
                      </td>
                      <td><button type="button" class="btn btn-danger btn-sm removeRow">Hapus</button></td>
                    </tr>
                    <?php endforeach; ?>
...
$("#addMainMaterial").click(function() {
    ...
    // Re-apply suggestion untuk row baru
    applySuggestion('.mm-part-list', '.partlist-suggestions', mmPartList);
    applySuggestion('.mm-material-spec', '.materialspec-suggestions', mmMaterialSpec);
});
...
// Fungsi untuk menentukan apakah input memiliki default value atau tidak
...
function hitungBerat($row) {
  ...
  $row.find('.mm-weight').val(berat.toFixed(1)); // ← ubah ke 1 angka di belakang koma
}
...
<script>
    // Buat array untuk part list dan material/spec
