...
use App\Models\DcpDesignProgramModel;
use App\Models\DcpPollyModel;
use App\Models\DcpMainMaterialModel;
use App\Models\DcpStandardPartModel;
use App\Models\DcpMachiningModel;
use App\Models\DcpFinishingModel;
use App\Models\DcpFinishing2Model;
use App\Models\DcpFinishing3Model;  
use App\Models\DcpHeatTreatmentModel;
use App\Models\DcpDieSpot1Model;
use App\Models\DcpDieSpot2Model;
use App\Models\DcpDieSpot3Model;
use App\Models\DcpToolCostModel;
use App\Models\DcpAksesorisModel;
use App\Models\CuttingToolModel;
use App\Models\FinishingModel;
use App\Models\DcpMachining2Model;
...
class Pps extends Controller
{
    ...
    public function index()
    {
        ...
        // Ambil semua data pps yang status = 1
        $pps = $ppsModel->findAll();
    
        // Grouping berdasarkan part_no
        $groupedPps = [];
        if (!empty($pps)) {
            foreach ($pps as $item) {
                $partNo = $item['part_no'] ?? 'UNKNOWN'; // kalau part_no kosong
    ...
    {
        $model = new PpsModel();
        $data = $model->find($id); // cari datanya dulu
    ...
    public function update()
    {
        ...
        if ($processLayout && $processLayout->isValid() && !$processLayout->hasMoved()) {
            ...
            // Pilih dari list → salin file
            $processLayoutSelected = urldecode($this->request->getPost('process_layout_selected')); // decode %20 jadi spasi
            $parsedPath = parse_url($processLayoutSelected, PHP_URL_PATH); // Ambil path saja
            $sourcePath = FCPATH . ltrim($parsedPath, '/'); // Gabungkan ke FCPATH
            ...
            if (file_exists($sourcePath)) {
                ...
                // Jika file tidak ditemukan, fallback ke gambar lama
...
if($totalDies >6){
        ...
        $mcCells = [
            'XX1', 'XX1', 'XX1', 'XX1', 'XX1', 'XX1',        // placeholder untuk index 0 & 1
    ...
    {
        // 1) Ambil maxOp dari query‐string
        ...
        foreach ($files as $file) {
            ...
            // 3) ambil 2 karakter pertama dari nama file (tanpa path)
            $prefix = substr($file, 0, 2);
    
            // 4) jika kedua karakter itu digit dan sama dengan maxOp
            if (ctype_digit($prefix) && intval($prefix) === $maxOp) {
                $images[] = base_url('uploads/process_layout/' . $file);
            }
        }
    
        // 5) kembalikan daftar gambar yang memenuhi
        return $this->response->setJSON($images);
    }
    ...
    public function listDieConsImg()
    {
        ...
        // Convert - to space untuk pencocokan nama file
        if ($process) {
            ...
            if (!empty($keywords)) {
                foreach ($keywords as $keyword) {
                    if ($keyword && stripos($file, $keyword) !== false) {
                        ...
                        break; // cukup satu keyword cocok
                    }
                }
            } else {
                $images[] = base_url('uploads/die_cons/' . $file);
   ...
   public function updateNew()
    {
        ...
        // 1) Jika user upload file baru, simpan itu
        if ($processLayoutFile && $processLayoutFile->isValid() && ! $processLayoutFile->hasMoved()) {
            ...
            $dataPps['process_layout'] = $processLayoutName;
        // 2) Jika user pilih dari list (hidden input berisi URL), copy file itu
        } elseif ($processLayoutSelected) {
            // ambil path relatif misal "uploads/process_layout/namafile.png"
        ...
        // 3) else: tidak upload dan tidak pilih → tetap pakai nama lama
        }else {
            $dataPps['process_layout'] = $this->request->getPost('old_process_layout');
        }
        ...
        if (!empty($dies) && is_array($dies)) {
            foreach ($dies as $index => $die) {
                ...
                $uploadPath = FCPATH . 'uploads/pps/'; // Sesuaikan dengan path folder pps
                
                if ($dieConstructionFile && $dieConstructionFile->isValid() && !$dieConstructionFile->hasMoved()) {
                    // Simpan file baru di folder uploads/pps
                    $dieConstrName = "die_constr_" . time() . "_$index." . $dieConstructionFile->getExtension();
                    ...
                    // Pastikan file tersebut ada
                    if (file_exists($sourcePath)) {
                        // Nama baru untuk gambar yang disalin
                        $dieConstrName = "die_constr_" . time() . "_$index." . pathinfo($sourcePath, PATHINFO_EXTENSION);
                
                        // Salin file ke folder tujuan (uploads/pps)
                    ...
                    // Jika tidak ada gambar yang di-upload dan tidak ada pilihan, gunakan gambar lama
                    $dieData['die_construction_img'] = $oldDieConstructionImg;
                }
