    ...
    public function generateExcel($pps_id)
    {
        ...
        ')
            ...
            if (!$firstOverview) {
               ...
               // Step 1: Cari sheet lama (OP10, OP20, OP30, dst)
            $worksheetName = 'OP' . $i . '0';
            $sheet = $spreadsheet->getSheetByName($worksheetName);

            if (!$sheet) {
                continue; // Skip kalau sheet belum ada
            }
            $usedSheets[] = $worksheetName; 
            // Step 2: Ganti nama sheet
            $newWorksheetName = $overview['process'] . (!empty($overview['process_join']) ? ', ' . $overview['process_join'] : '');

            $newWorksheetName = trim($newWorksheetName, ', -'); // Bersihkan jika ada koma/tanda minus diawal/akhir
            ...
            $session = session(); 
            $sheet->setCellValue('S9', $session->get('nickname'));
            ...
            // Cek nama file tidak kosong DAN file-nya benar-benar ada
