...
class History extends BaseController
{
    ...
    public function getCalendarEvents()
    {
        ...
        foreach ($izinData as $row) {
            ...
            $events[] = [
                ...
                'allDay' => true,
                'classNames' => [$className],
                'eventType' => 'izin',  // Add event type to distinguish
                'idEvent' => 'izin_' . $row['id'] // Add prefix to distinguish source
            ];
        }
    
        // Proses data libur (background merah + deskripsi)
        foreach ($holidayData as $holiday) {
            $tanggal = date('Y-m-d', strtotime($holiday['holiday_date']));
            ...
            // Jika ada deskripsi, tampilkan sebagai event
            if ($holiday['description'] != null) {
                $events[] = [
                    'title' => $holiday['description'],
                    'start' => $tanggal,
                    ...
                    'borderColor' => '#dc3545',
                    'eventType' => 'holiday',
                    'idEvent' => 'holiday_' . $holiday['id'] // Add prefix to distinguish source
                ];
            }
        }
    
        // Proses data dari tabel calendar
        foreach ($calendarData as $row) {
            $events[] = [
                'idEvent' => 'calendar_' . $row['id'], // Add prefix to distinguish source
                'title' => $row['title'],
                'start' => $row['date'],
                'notes' => $row['notes'],
                ...
                'eventType' => 'calendar' // Add event type to distinguish
            ];
        }
    ...
    public function profile()
    {
        ...
        // Ambil data user termasuk role_name
        ...
        // Ambil filter tanggal dari input (default 30 hari terakhir)
        $start_date = $this->request->getGet('start_date') ?: date('Y-m-d', strtotime('-30 days'));
        $end_date = $this->request->getGet('end_date') ?: date('Y-m-d');
    
        // Ambil data timeline berdasarkan rentang tanggal
